 ...
 * @Author: qicosmos 
 * @Version: v1.0 
 * @Date: 2014-02-21 14:44:00 
 * @Link: http://www.cnblogs.com/qicosmos/p/3559424.html
 * @Brief: 万能类型
 * @Last Modified by: cnscj
 * @Last Modified time: 2018-05-19 12:45:53
 * @Last Modified log: 
 */
#ifndef _SVARIANT_HPP_
#define _SVARIANT_HPP_
#include <typeindex>
#include <iostream>
#include <type_traits>
using namespace std;
...
public:
    ...
    SVariant(void) :m_typeIndex(typeid(void))
    {
    }
    ...
    SVariant(SVariant<Types...>&& old) : m_typeIndex(old.m_typeIndex)
    {
        Helper_t::move(old.m_typeIndex, &old.m_data, &m_data);
    }

    SVariant(const SVariant<Types...>& old) : m_typeIndex(old.m_typeIndex)
    {
        Helper_t::copy(old.m_typeIndex, &old.m_data, &m_data);
    }


　　　　Variant& operator=(const SVariant& old)
	　　　　{
		　　　　　　Helper_t::copy(old.m_typeIndex, &old.m_data, &m_data);
...
　　　　　　m_typeIndex = old.m_typeIndex;
    ...
    class = typename std::enable_if<Contains<typename std::remove_reference<T>::type, Types...>::value>::type>
        SVariant(T&& value) : m_typeIndex(typeid(void))
    {
            Helper_t::Destroy(m_typeIndex, &m_data);
